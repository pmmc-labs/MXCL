...
my $source = q[
    ...
    (diag ".chomp (build a real newline via chr(10))")
    (let newline (10 .chr))
    ...
    (diag ".split (str .split sep)")
    (let parts ("a,b,c" .split ","))
    (is (head parts)               "a" "... split head = a")
    (is (head (tail parts))        "b" "... split second = b")
    (is (head (tail (tail parts))) "c" "... split third = c")

    (diag ".join called on separator (sep .join list)")
    (let parts2 ("a,b,c" .split ","))
    (is ("," .join parts2) "a,b,c" "... join round-trips split")
    (is ("-" .join parts2) "a-b-c" "... join with different sep")
