...
test_mxcl(q[
    (let a  +["hello" "world"])
    (let r  (a .map (-> (s) (s .uc))))
    (is (r .at 0) "HELLO" "... map .uc [0] = HELLO")
    (is (r .at 1) "WORLD" "... map .uc [1] = WORLD")
]);

# --- .map identity: result equals original element-wise ---

test_mxcl(q[
    (let a  +[10 20 30])
    (let r  (a .map (-> (n) n)))
    (is (r .at 0) 10 "... identity map [0] = 10")
    (is (r .at 1) 20 "... identity map [1] = 20")
    (is (r .at 2) 30 "... identity map [2] = 30")
]);

# --- nested .map: apply .map to an array of arrays ---
